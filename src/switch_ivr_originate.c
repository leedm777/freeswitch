																...
																switch_dial_handle_list_t *hl)
{
	...
	*cause = SWITCH_CAUSE_SUCCESS;
	...
	for (i = 0; i < x_argc; i++) {
		...
		if (getcause && channel && cause && cause != SWITCH_CAUSE_SUCCESS) {
			switch_channel_handle_cause(channel, cause);
		}
		...
		if (getcause && *cause != cause && cause != SWITCH_CAUSE_LOSE_RACE && cause != SWITCH_CAUSE_NO_PICKUP) {
			*cause = cause;
			getcause++;
		}
